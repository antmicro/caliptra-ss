...
<%
  ...
  from lib.reggen.multi_register import MultiRegister
...
<%
      ...
      field_q_bits = bitarray(field_q_width, 2)
%>\
...
<%
      ...
      field_d_bits = bitarray(field_d_width, 2)
%>\
...
% for param_name, width in addr_widths.values():
  parameter int unsigned ${param_name} = ${width};
% endfor

  // Number of registers for every interface
% for iface_name, rb in block.reg_blocks.items():
  parameter int unsigned NumRegs${iface_name.title() if iface_name else ""} = ${len(rb.flat_regs)};
% endfor
<%
